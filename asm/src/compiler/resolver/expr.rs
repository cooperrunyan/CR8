...
impl Compiler {
    pub(crate) fn resolve_expr(&self, expr: &str) -> Result<i128, String> {
        let orig = expr;
        let expr = SYM_RE.replace_all(expr, |caps: &Captures| {
            let symbol = caps.get(0).unwrap().as_str().trim();

            if let Some(stat) = self.statics.get(symbol) {
                (stat.to_owned() as i128).to_string()
            } else if let Some(label) = self.labels.get(symbol) {
                (label.to_owned() as i128).to_string()
            } else if let Some(label) = self.labels.get(&format!("{}{}", self.last_label, symbol)) {
                (label.to_owned() as i128).to_string()
            } else if let Some(ram_loc) = self.ram_locations.get(symbol) {
                ((*ram_loc as i128) + self.ram_origin as i128).to_string()
            } else {
                symbol.to_string()
            }
        });

        let expr = DOT_RE.replace(&expr, self.bin.len().to_string());
    ...
    fn evaluate_grp(&self, expr: &str) -> Result<i128, String> {
        ...
        if let Some(g) = groups {
            for group in g.iter() {
                ...
                let res = self.evaluate_grp(&group.as_str()[1..end])?;
